...
// 2. Current CPU usage
func (hc *Server) cpu_usage(w http.ResponseWriter, req *http.Request) {
	hc.grab_counter(w)
...
func (hc *Server) cpu_hour_usage(w http.ResponseWriter, req *http.Request) {
	hc.grab_counter(w)
	avg := Cpu_avgh_.get_cpu_avg()
	fmt.Fprintf(w, "CPU usage over last hour is: %3.2f\n", avg)
	hc.release_counter()
}
